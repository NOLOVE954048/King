...
from Vip import BOT_TOKEN, ADMIN_IDS
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id not in users:
        await update.message.reply_text("❌ You don't have an active subscription. Please contact the admin for assistance.Buy Form @BOT_HU_ME_BHAI", reply_markup=main_menu_keyboard())
    else:
        expiration_date = users[user_id]
        await update.message.reply_text(f"👋 Welcome {username}!\n Your subscription is active until {expiration_date}.\n This Tool is provided by @BOT_HU_ME_BHAI", reply_markup=main_menu_keyboard())
...
async def bgmi(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(context.args) != 3:
        await update.message.reply_text("🛡️ Usage: /soul <target_ip> <port> <duration>")
        return
    ...
    command = ['./soul', target_ip, str(port), str(duration)]
    try:
...
async def genkey(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in ADMIN_IDS:
        ...
        response = "ONLY OWNER CAN USE💀OWNER @BOT_HU_ME_BHAI"
...
async def redeem(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if len(command) == 1:
        ...
        if key in keys:
            ...
            response = "Invalid or expired key buy from @KaliaYtOwner."
    else:
